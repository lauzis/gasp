...
class PanelIndicator extends PanelMenu.Button {
    ...
    _updateMenuItems() {
        ...
        const isLiveRecord = currentLive > 0 && currentLive >= recordLive;
        ...
        this._liveItem.setIcon(isLiveRecord ? trophyIcon : this._getProgressIcon(currentLive, recordLive));
        this._setMenuItemIconSize(this._liveItem);
        this._liveItem.label.set_text(
            `Live: ${recordLive} / ${currentLive}`
        );
        
        this._dailyItem.setIcon(isDailyRecord ? trophyIcon : this._getProgressIcon(currentDaily, recordDaily));
        this._setMenuItemIconSize(this._dailyItem);
        this._dailyItem.label.set_text(
            `Daily: ${recordDaily} / ${currentDaily}`
        );
        
        this._weeklyItem.setIcon(isWeeklyRecord ? trophyIcon : this._getProgressIcon(currentWeekly, recordWeekly));
        this._setMenuItemIconSize(this._weeklyItem);
        this._weeklyItem.label.set_text(
            `Weekly: ${recordWeekly} / ${currentWeekly}`
        );
        
        this._monthlyItem.setIcon(isMonthlyRecord ? trophyIcon : this._getProgressIcon(currentMonthly, recordMonthly));
    ...
    _getProgressIcon(current, peak) {
        const percentage = this._calculateProgressPercentage(current, peak);
        const rounded = Math.round(percentage);

        if (this._pieIconCache.has(rounded)) {
            return this._pieIconCache.get(rounded);
        }

        try {
            GLib.mkdir_with_parents(this._pieIconDir, 0o700);
            const iconPath = GLib.build_filenamev([this._pieIconDir, `pie-${rounded}.svg`]);
            const iconFile = Gio.File.new_for_path(iconPath);

            if (!iconFile.query_exists(null)) {
                GLib.file_set_contents(iconPath, this._buildPieSvg(rounded));
            }

            const icon = this._createIcon(iconPath);
            this._pieIconCache.set(rounded, icon);
            return icon;
        } catch (e) {
    ...
    _calculateProgressPercentage(current, peak) {
        ...
        return Math.max(0, Math.min(100, (safeCurrent / safePeak) * 100));
    }

    _buildPieSvg(percentage) {
        ...
        if (pct >= 100) {
            fillSector = `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="${color}" fill-opacity="0.95"/>`;
        } else if (pct > 0) {
            ...
            fillSector = `<path d="M ${cx} ${cy} L ${cx} ${cy - radius} A ${radius} ${radius} 0 ${largeArcFlag} 1 ${x.toFixed(3)} ${y.toFixed(3)} Z" fill="${color}" fill-opacity="0.95"/>`;
        }

        return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
  <circle cx="${cx}" cy="${cy}" r="${radius}" fill="${color}" fill-opacity="0.2"/>
  ${fillSector}
  <circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${color}" stroke-opacity="0.75" stroke-width="1"/>
</svg>`;
    }
