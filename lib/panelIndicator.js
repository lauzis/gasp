...
class PanelIndicator extends PanelMenu.Button {
    _init(uuid, name, settings, onSettingsClick, onForceRefresh, logger, extensionPath) {
        ...
        this._hasCredentialsCache = null;
    ...
    _connectSignals() {
        ...
        this._settingsChangedId = this._settings.connect('changed', (settings, key) => {
            if (key === SETTINGS_KEYS.GA_API_KEY ||
                key === SETTINGS_KEYS.GA_PROPERTY_ID ||
                key === SETTINGS_KEYS.CREDENTIAL_STORAGE) {
                this._invalidateCredentialsCache();
            }
    ...
    _updateMenuItems() {
        ...
        const hasCredentials = this._getHasCredentials();
        ...
        const liveTopRecord = Math.max(totalLivePeak, currentLive);
        ...
        this._liveItem.setIcon(
            isLiveRecord
                ? trophyIcon
                : this._getProgressIcon('live', todayLivePeak, liveTopRecord, 100)
        );
        this._setMenuItemIconSize(this._liveItem);
        this._liveItem.label.set_text(
            `Live: ${liveTopRecord} / ${todayLivePeak} / ${currentLive}`
        );
    ...
    _invalidateCredentialsCache() {
        this._hasCredentialsCache = null;
    }

    _getHasCredentials() {
        if (this._hasCredentialsCache !== null) {
            return this._hasCredentialsCache;
        }

        const hasStoredCredentials = Boolean(loadCredentials(this._settings));
        const propertyId = this._settings.get_string(SETTINGS_KEYS.GA_PROPERTY_ID).trim();
        this._hasCredentialsCache = hasStoredCredentials && propertyId.length > 0;
        return this._hasCredentialsCache;
    }
    ...
    _getProgressIcon(period, visitCount, peakVisitCount, periodProgressOverride = null) {
        const periodProgress = periodProgressOverride ?? this._calculatePeriodProgressPercentage(period);
