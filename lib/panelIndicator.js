...
class PanelIndicator extends PanelMenu.Button {
    ...
    _getProgressIcon(period, visitCount, peakVisitCount) {
        ...
        const roundedProgress = Math.round(periodProgress);
        const cacheKey = `${roundedProgress}-${pieColor}`;
        ...
        try {
            ...
            const iconPath = GLib.build_filenamev([this._pieIconDir, `pie-${roundedProgress}-${colorToken}.svg`]);
            const iconFile = Gio.File.new_for_path(iconPath);

            if (!iconFile.query_exists(null)) {
                GLib.file_set_contents(iconPath, this._buildPieSvg(roundedProgress, pieColor));
            }
