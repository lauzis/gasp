...
import {
    ...
    getPreviousIsoWeekRangeDateStrings,
    getPreviousMonthRangeDateStrings,
} from './dateUtils.js';
...
export class GAClient {
    ...
    async fetchStats() {
        ...
        if (propertyId !== this._recordSeedPropertyId) {
            ...
            this._recordSeeded = { daily: false, weekly: false, monthly: false };
            this._settings.set_string(SETTINGS_KEYS.ANALYTICS_TIMEZONE, '');
        }
        ...
        try {
            ...
            const hasAnalyticsTimeZone = Boolean(analyticsTimeZone);
            const today = hasAnalyticsTimeZone ? getDateString(now, analyticsTimeZone) : 'today';
            const weekStart = hasAnalyticsTimeZone
                ? getIsoWeekStartDateString(now, analyticsTimeZone)
                : getDateString(getIsoWeekStart(now));
            const monthStart = hasAnalyticsTimeZone
                ? getMonthStartDateString(now, analyticsTimeZone)
                : getDateString(getMonthStart(now));
            ...
            let [liveStats, dailyStats, weeklyStats, monthlyStats] = await Promise.all([
                ...
                this._fetchPeriodStats(propertyId, token, weekStart, today),
                this._fetchPeriodStats(propertyId, token, monthStart, today)
            ]);
            ...
            // Seed records if needed, without replacing current-period values.
            ...
            return {
                ...
                timeZone: this._getAnalyticsTimeZone(),
            };
    ...
    async _fetchPreviousPeriod(period, propertyId, token, now) {
        ...
        const ranges = {
            daily: () => {
                const previousDate = getPreviousDayDateString(now, analyticsTimeZone);
                return [previousDate, previousDate];
            },
            weekly: () => {
                const previousRange = getPreviousIsoWeekRangeDateStrings(now, analyticsTimeZone);
                return [previousRange.start, previousRange.end];
            },
            monthly: () => {
                const previousRange = getPreviousMonthRangeDateStrings(now, analyticsTimeZone);
                return [previousRange.start, previousRange.end];
            }
        };
    ...
    async _signRSA(data, privateKeyPem) {
        try {
            ...
            try {
                ...
                dataPath = tmpPath;
                GLib.close(dataFd);
                this._writeSecureFile(dataPath, data);
            ...
            let sigFile = null;
            try {
                keyFile = this._createSecureTempPath('gasp_key_XXXXXX');
                dataFile = this._createSecureTempPath('gasp_data_XXXXXX');
                sigFile = this._createSecureTempPath('gasp_sig_XXXXXX');

                this._writeSecureFile(keyFile, privateKeyFixed);
                this._writeSecureFile(dataFile, data);
                ...
                return this._base64UrlEncodeBytes(sigBytes);
            } finally {
                this._cleanupTempFiles([keyFile, dataFile, sigFile].filter(Boolean));
            }
    ...
    _createSecureTempPath(template) {
        const [fd, path] = GLib.file_open_tmp(template);
        GLib.close(fd);
        return path;
    }

    _writeSecureFile(path, contents) {
        GLib.file_set_contents_full(path, contents, GLib.FileSetContentsFlags.NONE, 0o600);
    }
    ...
    _getAnalyticsTimeZone() {
        const timeZone = this._settings.get_string(SETTINGS_KEYS.ANALYTICS_TIMEZONE);
        return timeZone || null;
    }

    _updateAnalyticsTimeZoneFromResponse(response) {
        const reportedTimeZone = response?.metadata?.timeZone;
        if (!reportedTimeZone || typeof reportedTimeZone !== 'string') {
            return;
        }

        const currentTimeZone = this._settings.get_string(SETTINGS_KEYS.ANALYTICS_TIMEZONE);
        if (currentTimeZone === reportedTimeZone) {
            return;
        }

        this._settings.set_string(SETTINGS_KEYS.ANALYTICS_TIMEZONE, reportedTimeZone);
        this._logger.info(`Analytics timezone detected: ${reportedTimeZone}`);
    }
    ...
    async _fetchPeriodStats(propertyId, token, startDate, endDate) {
        try {
            ...
            this._updateAnalyticsTimeZoneFromResponse(response);
